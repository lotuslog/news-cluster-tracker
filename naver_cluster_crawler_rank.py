...
TBL_RANK_HISTORY = "rank_history"  # [임시 실험용] 클러스터 내 기사 순위 변동 추적
...
# [임시 실험용] 한 클러스터 안에서 기사 순위(rank)가 시간에 따라 어떻게
# 바뀌는지 보기 위한 append-only 로그. article_cluster_link의
# initial_rank는 "최초 진입 시" 1회만 기록되어 변동 추적이 불가능하므로,
# 매 수집 사이클마다 그 시점의 rank를 그대로 쌓는 별도 테이블을 둔다.
# UPSERT 없이 단순 append만 한다 — 운영 파이프라인(article_cluster_link)과
# 완전히 분리된 실험용 테이블이라 추후 필요 없어지면 그냥 DROP하면 된다.
SCHEMA_RANK_HISTORY = [
    bigquery.SchemaField("cluster_id",   "STRING",   mode="REQUIRED"),
    bigquery.SchemaField("article_url",  "STRING",   mode="REQUIRED"),
    bigquery.SchemaField("rank",         "INTEGER",  mode="REQUIRED"),
    bigquery.SchemaField("observed_at",  "DATETIME", mode="REQUIRED"),
]
...
def ensure_tables(client: bigquery.Client) -> dict[str, str]:
    ...
    table_configs = [
        ...
        (TBL_RANK_HISTORY, SCHEMA_RANK_HISTORY,    "observed_at"),    # [임시 실험용]
    ]
...
def main():
    ...
    rank_history_id = table_ids[TBL_RANK_HISTORY]  # [임시 실험용]
    ...
    with sync_playwright() as pw:
        ...
        for section_name, section_id in sections.items():
            try:
                ...
                # 5-1. [임시 실험용] 이번에 본 모든 (cluster_id, article_url)의
                # 현재 rank를 그대로 append — 운영 로직과 무관, 순수 관찰용
                rank_rows = [
                    {
                ...
                bq_insert(client, rank_history_id, SCHEMA_RANK_HISTORY, rank_rows)
